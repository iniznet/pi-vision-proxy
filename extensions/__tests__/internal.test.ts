...
describe("envFlags", () => {
	it("reports presence per variable", () => {
		assert.deepEqual(envFlags({}), { mode: false, model: false, context: false, tool: false, maxImagesPerCall: false, maxBatch: false, cacheSize: false, autoConsent: false });
		assert.deepEqual(
			envFlags({
			...
			{ mode: true, model: true, context: true, tool: false, maxImagesPerCall: false, maxBatch: false, cacheSize: false, autoConsent: false },
		);
	});
});
